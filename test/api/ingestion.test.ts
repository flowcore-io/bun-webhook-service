...
beforeAll(async () => {
  ...
  // Connect to real NATS server with retry
  ...
  // Connect to real Redis Sentinel with retry
  let redisConnected = false;
  for (let i = 0; i < 10; i++) {
    try {
      await redisFixture.connect(
        REDIS_SENTINEL_HOSTS.split(","),
        REDIS_MASTER_NAME,
      );
      redisConnected = true;
      break;
  ...
  // App and auth fixtures are already started by global setup
  // Just ensure authorized user is set up
...
afterAll(async () => {
  // Don't stop app/auth fixtures - they're managed by global setup
...
beforeEach(async () => {
  // Clean up test data - wrap in Promise.race to prevent hanging
  const cleanupPromise = Promise.all([
    testDb.truncateAll(),
    redisFixture.clear().catch(() => {
      // Ignore Redis errors during cleanup - non-critical
    }),
  ]);
  
  // Add timeout to cleanup to prevent hook timeout
  await Promise.race([
    cleanupPromise,
...
describe("POST /api/v1/event/:tenant/:dataCore/:flowType/:eventType", () => {
  test("should ingest valid event and publish to NATS", async () => {
    ...
    const collectMessages = natsFixture.subscribe(
      TOPIC_GUARANTEED_INGESTION_CHANNEL,
    );
...
describe("POST /api/v1/events/:tenant/:dataCore/:flowType/:eventType", () => {
  ...
  test("should handle headers: X-Flowcore-Metadata-Json", async () => {
    ...
    const metadataJson = Buffer.from(JSON.stringify(customMetadata)).toString(
      "base64",
    );
  ...
  test("should handle all headers combined", async () => {
    ...
    // Give subscription a moment to be fully active
    ...
    // Wait a bit longer to ensure message is published and received
